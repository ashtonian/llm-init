# test-project Development Infrastructure

This document describes the local infrastructure services required by MCP servers and the test-project platform, and how agents should manage them.

## Services

| Service    | Container Name            | Port(s)      | Purpose                          |
|------------|---------------------------|--------------|----------------------------------|
| PostgreSQL | test-project-postgres | 5432         | Primary database                 |
| Redis      | test-project-redis    | 6379         | L2 cache layer                   |
| NATS       | test-project-nats     | 4222, 8222   | Message bus (JetStream enabled)  |

### Credentials

| Service    | Connection String                                                      |
|------------|------------------------------------------------------------------------|
| PostgreSQL | `postgresql://test-project:test-project@localhost:5432/test-project` |
...
```bash
# PostgreSQL
docker exec test-project-postgres pg_isready -U test-project -d test-project

# Redis
docker exec test-project-redis redis-cli ping
