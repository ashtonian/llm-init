...
for i in $(seq 1 "$NUM_AGENTS"); do
    ...
    ROLE_INDEX=$((i - 1))
    AGENT_ROLE_ENV=""
    if [[ ${#ROLE_ARRAY[@]} -gt 0 ]] && [[ $ROLE_INDEX -lt ${#ROLE_ARRAY[@]} ]]; then
        AGENT_ROLE_ENV="${ROLE_ARRAY[$ROLE_INDEX]}"
        echo "Starting ${AGENT_NAME} (role: ${AGENT_ROLE_ENV})..."
    else
        echo "Starting ${AGENT_NAME}..."
    fi
    AGENT_ROLE="${AGENT_ROLE_ENV}" bash "${SCRIPT_DIR}/run-agent.sh" "$AGENT_NAME" > "$LOG_FILE" 2>&1 &
    PID=$!
