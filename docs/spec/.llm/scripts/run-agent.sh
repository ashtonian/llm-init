...
build_prompt() {
    ...
    if [[ -n "$AGENT_ROLE" ]]; then
        local role_file="${HARNESS_DIR}/templates/roles/${AGENT_ROLE}.md"
        if [[ -f "$role_file" ]]; then
...
13. **Curate patterns**: After completing work, review the Iteration Log for patterns that would
    help future agents. Consolidate reusable learnings into the **Codebase Patterns** section
    at the top of PROGRESS.md. Deduplicate — if a similar pattern exists, refine it instead
    of adding a duplicate.
14. **Distributed context**: If you work extensively in a subdirectory, consider creating
    or updating a \`CLAUDE.md\` file there with directory-specific patterns and conventions
    (keep under 30 lines). Claude Code auto-reads these for future sessions.
15. **Context hygiene**: Redirect verbose command output (build logs, test output) to log files
    in \`docs/spec/.llm/logs/\`. Keep your working context focused on the current task — don't
    read unrelated task files or specs.
...
execute_task() {
    ...
    if [[ $exit_code -eq 0 ]] && grep -q "TASK_COMPLETE" "$log_file"; then
        ...
        if git merge "$branch_name" --no-ff -m "feat: complete ${task_name} [${AGENT_NAME}]" 2>/dev/null; then
            ...
            log "DONE: task=${task_name} agent=${AGENT_NAME}"
        else
            log "ERROR: merge_failed task=${task_name} branch=${branch_name}"
...
EOF
        ...
        log "WARN: task_blocked task=${task_name} reason=${reason}"
        mv "${TASKS_DIR}/in_progress/$(basename "$task_file")" "${TASKS_DIR}/blocked/"
        release_task "$task_name"
...
while true; do
    ...
    if [[ -z "${TASK_FILE:-}" ]]; then
        ...
        if [[ "$IN_PROGRESS" -eq 0 ]] && [[ "$BACKLOG" -eq 0 ]]; then
            COMPLETED_COUNT=$(ls "${TASKS_DIR}/completed/"*.md 2>/dev/null | wc -l | tr -d ' ')
            log "All tasks complete! Agent shutting down."
            log "DONE: all_tasks_complete total=${COMPLETED_COUNT} agent=${AGENT_NAME}"
